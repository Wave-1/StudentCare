...
const FormResultsAnswered = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            let apiUrl = `${API_BASE_URL}${API_ROUTES.ResultsAnswered}`;
    ...
    const deleteUser = (resultsAnsweredID) => {
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.value) {
                deleteApi(resultsAnsweredID);
            }
        });
    };

    const deleteApi = async (resultsAnsweredID) => { // Change 'id' to 'teacherID'
        try {
            const response = await axios.delete(`${API_BASE_URL}${API_ROUTES.ResultsAnswered}/${resultsAnsweredID}`, {
                headers: API_HEADERS
            }); // Corrected the URL format

            if (response.status === 200) { // Check the status code, not 'ok'
                const updatedRows = rows.filter((row) => row.resultsAnsweredID !== resultsAnsweredID);
                setRows(updatedRows);
    ...
    const filterData = (v) => {
        if (v) {
            ...
            axios.get(`${API_BASE_URL}${API_ROUTES.ResultsAnswered}?teacherID=${teacherID}`,
                { headers: API_HEADERS })
                .then((response) => {
                    if (response.status === 200) {
            ...
            <Paper className="w-full overflow-hidden p-3">
                ...
                <Stack direction="row" spacing={2} className="my-2 mb-2">
                    <Autocomplete
                        ...
                        getOptionLabel={(row) => `${row.studentID}: ${row.problemName || ""}`}
                        renderInput={(params) => (
