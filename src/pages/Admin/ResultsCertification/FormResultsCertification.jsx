...
const FormResultsCertification = () => {
    ...
    const deleteUser = (resultsCertificationID) => {
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.value) {
                deleteApi(resultsCertificationID);
            }
        });
    };

    const deleteApi = async (resultsCertificationID) => { // Change 'id' to 'teacherID'
        try {
            const response = await axios.delete(`${API_BASE_URL}${API_ROUTES.ResultsCertification}/${resultsCertificationID}`, {
                headers: API_HEADERS
            }); // Corrected the URL format

            if (response.status === 200) { // Check the status code, not 'ok'
                const updatedRows = rows.filter((row) => row.resultsCertificationID !== resultsCertificationID);
                setRows(updatedRows);
    ...
    useEffect(() => {
        const fetchData = async () => {
            let apiUrl = `${API_BASE_URL}${API_ROUTES.ResultsCertification}`;
    ...
    const filterData = (v) => {
        if (v) {
            ...
            axios.get(`${API_BASE_URL}${API_ROUTES.ResultsCertification}`,
                { headers: API_HEADERS })
                .then((response) => {
                    if (response.status === 200) {
                        return response.data;
                                            ...
                                            <TableCell className="min-w-100">
                                                <Stack spacing={2} direction="row">
                                                    ...
                                                    <DeleteIcon
                                                        ...
                                                        onClick={() => {
                                                            deleteUser(row.resultsCertificationID);
                                                        }}
                                                    />
                                                </Stack>
                                            </TableCell>
